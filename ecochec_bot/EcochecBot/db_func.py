...
def add_device(city_id: str):
    ...
    cursor.execute(f'INSERT INTO devices (device_id, city_id, t, g, s, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?)',
                   (device_id, city_id, 'нет данных', 'нет данных', 'нет данных', 'нет данных', 'нет данных'))
...
def set_data_to_device(device_id: str, t: str, g: str, s: str, latitude: str, longitude: str):
    try:
        cursor.execute(f'UPDATE devices SET t = ?, g = ?, s = ?, latitude = ?, longitude = ? WHERE device_id = ?',
                    (t, g, s, latitude, longitude, device_id))
        conn.commit()
        return 'OK'
    except:
        return 'ERROR'
